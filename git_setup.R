...
if(!is.null(attr(suppressWarnings(system('git config alias.upd',intern=T))
                 ,'status'))){
  system('git config --global alias.upd "!git submodule update --init --recursive --remote && git submodule foreach -q --recursive \'git checkout \\$(git config -f \\$toplevel/.gitmodules submodule.\\$name.branch || echo master)\'"')
         } else message(
